...
namespace 日报模板
{
    public partial class Repairtestlookup : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)      //查找
        {
            ...
            if (dateTimePicker1.Text == dateTimePicker2.Text)
            {
                if (textBox1.Text == "")
                {
                    sql = "select * from repairtest order by time desc";
                }
                else
                {
                    sql = "select * from repairtest where " + comboBox1.Text + " ='" + textBox1.Text + "'";
                }
            }
            else if (textBox1.Text != "")
            {
                sql = "select * from repairtest where " + comboBox1.Text + " ='" + textBox1.Text + "' and time > ='" + dateTimePicker1.Value + "' and time < '" + dateTimePicker2.Value + "'order by time";
            }

            else
            {
                sql = "select * from repairtest where  time > ='" + dateTimePicker1.Value + "' and time < '" + dateTimePicker2.Value + "'order by time";
            }
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Repairtestrevise re = new Repairtestrevise(obj);
            re.Show();
        }
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (logon.name == "王丽霞")
            {
                try
                {
                    ...
                    string delete_by_id = "delete from repairtest where ID=" + select_id + "";
        ...
        public void lookup()
        {
            dataGridView1.DataSource = null;
            SqlConnection conn = Class1.GetSqleverconn();
            string sql = "select * from repairtest order by time desc";
            SqlDataAdapter data = new SqlDataAdapter(sql, conn);
            DataSet ds = new DataSet();
            data.Fill(ds);
            Class1.CloseSqlServerConn(conn);
        ...
        public void look()
        {
            ...
            dataGridView1.Columns[4].HeaderCell.Value = "编号";
            dataGridView1.Columns[5].HeaderCell.Value = "持有人";
            dataGridView1.Columns[6].HeaderCell.Value = "mac地址";
            dataGridView1.Columns[7].HeaderCell.Value = "固件版本号";
            dataGridView1.Columns[8].HeaderCell.Value = "现象描述";
            dataGridView1.Columns[9].HeaderCell.Value = "备注";
