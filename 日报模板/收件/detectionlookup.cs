...
namespace 日报模板
{
    public partial class detectionlookup : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)      //查找
        {
            ...
            if (dateTimePicker1.Text == dateTimePicker2.Text)
            {
                if (textBox1.Text == "")
                {
                    sql = "select * from detection order by time desc";
                }
                else
                {
                    sql = "select * from detection where " + comboBox1.Text + " ='" + textBox1.Text + "'";
                }
            }
            else if (textBox1.Text != "")
            {
                sql = "select * from detection where " + comboBox1.Text + " ='" + textBox1.Text + "' and time > ='" + dateTimePicker1.Value + "' and time < '" + dateTimePicker2.Value + "'order by time";
            }

            else
            {
                sql = "select * from detection where  time > ='" + dateTimePicker1.Value + "' and time < '" + dateTimePicker2.Value + "'order by time";
            }
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            detectionrevise re = new detectionrevise(obj);
            re.Show();
        }
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (logon.name == "王丽霞")
            {
                try
                {
                    ...
                    string delete_by_id = "delete from detection where ID=" + select_id + "";
        ...
        public void lookup()
        {
            dataGridView1.DataSource = null;
            SqlConnection conn = Class1.GetSqleverconn();
            string sql = "select * from detection order by time desc";
            SqlDataAdapter data = new SqlDataAdapter(sql, conn);
            DataSet ds = new DataSet();
            data.Fill(ds);
            Class1.CloseSqlServerConn(conn);
